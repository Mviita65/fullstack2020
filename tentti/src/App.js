...
function Vaihtoehdot (props) { // näytölle kysymysten vaihtoehdot ja reagointi jos merkitään vastaukseksi
  ...
  return <section>{kopioData[props.tenttiIndex].kysymykset[props.kysymysIndex].vaihtoehdot.map((item,veIndex) =>
...
function Tentti (props) {  //näytölle tentin kysymykset ja kutsuu kysymysten vaihtoehdot

  let kopioData = JSON.parse(JSON.stringify(props.data))
  return <section>{kopioData[props.tenttiIndex].kysymykset.map((item,kysymysIndex) => 
    <div key={kysymysIndex} className="kysymys" >{item.kysymys} <Vaihtoehdot vastausVaihtui={props.vastausVaihtui} tenttiIndex={props.tenttiIndex} kysymysIndex={kysymysIndex} data={kopioData}/>
    </div>)}</section>
}
  ...
  return (
    <div className="grid-container">
      <nav className="sovellusvalikko">
        <span className="s-nav-item" onClick={e => {setTentit(1);setTietoa(0)}}>TENTIT </span>
       ...
       tentit ? 
        <div>
          <nav className="tenttivalikko">
            <span className="t-nav-item" onClick={e => {setTenttiA(1);setTenttiB(0)}}>TENTTI A </span>
            <span className="t-nav-item" onClick={e => {setTenttiB(1);setTenttiA(0)}}>TENTTI B </span>
          </nav>
